...
(deftest counter
  ...
  (is (= 20 (stm/deref sum)))))
  ; This should succeed in any implementation.
...
(deftest summer-send
  (let [contentious-ref (stm/ref 0)
    ...
    (actor/send receiver-actor :check) (Thread/sleep 10)
    ; This fails in the default implementation.
    ; Since send is not reverted when a transaction is reverted, its effects
    ; remain visible after a rollback, and receiver-actor's i may be > 100.
    (is @check-success?))) ; ensure that test actually executed
...
(deftest summer-spawn
  (let [sum (stm/ref 0)
    ...
    (doseq [s spawners]
      (actor/send s :do))
    (Thread/sleep 1000)
    (is (= 100 (stm/deref sum)))))
    ; This fails in the default implementation.
    ; Since spawn is not reverted when a transaction is reverted, its effects
    ; remain visible after a rollback, and sum may be > 100.
...
(deftest flagger-become
  (let [one-flag-set? (stm/ref false)
    ...
    (let [n (count @flags-at-the-end)
      ...
      ; transaction is undone, and > 1 flag may be set.
